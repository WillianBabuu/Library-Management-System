        ...
        <div class="flex flex-col pt-5">
            <base-table-wrap>
                <base-table>
                    ...
                    <tbody>
                        <tr v-for="(item, indexItem) in listOfPurchase" :key="indexItem" :class="(indexItem % 2 == 0) ? 'bg-white':'bg-gray-50'">
                ...
                <base-table-no-results-found v-if="!listOfPurchase.length"/>
            </base-table-wrap>
        </div>
    </div>
</template>
...
<script>
    ...
    import returningService from "@services/returning-service.js";
    ...
    export default {
        ...
        methods: {
            async searchReturning() {
                returningService.search({q: ''}, (result)=>{
                    this.listOfPurchase = result.returnData.list_of_items;
                });
            },
            async toWaitingRecievings(){
                this.$router.push({ name: 'returning-list' });
            },
            async goToDetailsReturning(id){
                this.$router.push({ name: 'returning-details', params: { id:id } });
            },
            async deleteReturning(id){
                uiService.confirmDelete(()=>{
                    returningService.delete(id, (results)=>{
