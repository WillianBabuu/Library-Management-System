<template>
    <div class="flex flex-wrap justify-between lg:flex-nowrap mb-5">
        <div>
            <h6 class="font-medium text-lg text-left">Attributes</h6>
            <p class="mt-2 text-sm leading-snug text-left text-gray-500 max-w-[680px]">
                List of Attributes
            </p>
        </div>
        <div class="mt-4 lg:mt-0 lg:ml-2">
            <base-btn-outline @click="this.$refs.slideOverDialogAttribute.setIsOpen(true)"
                btn-icon="PlusIcon" btn-label="Add Store" btn-color="indigo"/>
    ...
    <div class="flex flex-col pt-5">
        <base-table-wrap>
            <base-table>
                ...
                <tr>
                    <base-table-header>Full Name</base-table-header>
                    <base-table-header>Attribute Group</base-table-header>
                    <base-table-header>Value</base-table-header>
                    <base-table-header-end>Action</base-table-header-end>
                </tr>
                </thead>
                <tbody>
                    <tr v-for="(item, indexItem) in listOfAttribute" :key="indexItem" :class="(indexItem % 2 == 0) ? 'bg-white':'bg-gray-50'">
                        <base-table-data>
                            <span v-if="item.name">{{ item.name }}</span>
                            <span v-else>Not Set</span>
                        </base-table-data>
                        <base-table-data>
                            <span v-if="item.attribute_group_id">{{ item.attribute_group.name }}</span>
                            <span v-else>Not Set</span>
                        </base-table-data>
                        <base-table-data>
                            ...
                            <base-table-data>
                            <span v-if="item.group_id">{{ item.group_id }}</span>
                            <span v-else>Not Set</span>
                        </base-table-data>
            ...
            <base-table-no-results-found v-if="!listOfAttribute.length"/>
        </base-table-wrap>
    </div>
    <slide-over-dialog-attribute ref="slideOverDialogAttribute" @refreshAttributes="searchAttribute"/>
</template>
...
<script>
    ...
    import SlideOverDialogAttribute from "@views/console/slide-over-dialog/SlideOverDialogAttribute";
    export default {
        name: 'attribute-list',
        components: {
            ...
            BaseBtn, BaseBtnOutline,
            BaseBtnContainer,
            BaseConsoleBreadcrumb,
            BaseTableNoResultsFound,SlideOverDialogAttribute,
        },
        data() {
            return {
                listOfAttribute: [],
                form: {
                    id: '', form_method: 'save', name: '', value: '', group: ''                }
            }
        },
        async created() {
        ...
        methods: {
            async searchAttribute() {
                attributeService.search({q:''}, (result)=>{
                    this.listOfAttribute = result.returnData.list_of_items;
                });
            },
            async goToDetailsAttribute(id){
                this.$refs.slideOverDialogAttribute.setIsOpen(true, id);
            },
            async deleteAttribute(id){
                uiService.confirmDelete(()=>{
                    attributeService.delete(id, (results)=>{
