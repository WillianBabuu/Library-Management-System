        ...
        <div class="flex flex-col pt-5">
            <base-table-wrap>
                <base-table>
                    ...
                    <tbody>
                        <tr v-for="(item, indexItem) in listOfReturninginward" :key="indexItem" :class="(indexItem % 2 == 0) ? 'bg-white':'bg-gray-50'">
                                                ...
                                                <MenuItems
                                                    ...
                                                    <div class="px-1 py-1">
                                                        <MenuItem v-slot="{ active }">
                                                            <button @click="goToDetailsReturninginward(item.id)"
                                                                :class="[
                                                    ...
                                                    <div class="px-1 py-1">
                                                        <MenuItem v-slot="{ active }">
                                                            <button @click="deleteReturninginward(item.id)"
                                                                :class="[
                                                                        active ? 'bg-indigo-500 text-white' : 'text-gray-900',
                                                                        'group flex rounded-md items-center w-full px-2 py-2 text-sm',
                                                                    ]">
                ...
                <base-table-no-results-found v-if="!listOfReturninginward.length"/>
            </base-table-wrap>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                listOfReturninginward: [],
            }
        },
        async created() {
            // console.log('About to get Item List');
            await this.searchReturninginward();
        },
        methods: {
            async searchReturninginward() {
                returninginwardService.search({q: ''}, (result)=>{
                    this.listOfReturninginward = result.returnData.list_of_items;
                });
            },
            async toWaitingRecievings(){
                this.$router.push({ name: 'returninginward-list' });
            },
            async goToDetailsReturninginward(id){
                this.$router.push({ name: 'returninginward-details', params: { id:id } });
            },
            async deleteReturninginward(id){
                uiService.confirmDelete(()=>{
                    returninginwardService.delete(id, (results)=>{
                        uiService.showSuccess('Deleted!', results.errorMessage);
                        this.$forceUpdate()
                        this.searchReturninginward()
                    });
                });
            },
        },
