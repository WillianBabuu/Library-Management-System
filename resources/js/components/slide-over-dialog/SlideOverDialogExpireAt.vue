                    ...
                    <form class="flex-1 flex flex-col" @submit.prevent="savePriceQuantity">
                        ...
                        <div class="flex-1 relative">
                            ...
                            <div class="absolute inset-0 overflow-y-scroll  py-6 px-4">
                                ...
                                <div class="relative w-full group" v-if="listPrices.selling_price">
                                    <label for="type" class="block mb-2 text-sm font-medium text-gray-800">
                                       Last Buying Price was: {{ listPrices.buying_price }}
                                    </label>
                                    <label for="type" class="block mb-2 text-sm font-medium text-gray-800">
                                       Last Selling Price was: {{ listPrices.selling_price }} 
                                    </label>
                                </div>
                                <hr>
                                <br>
                                ...
                                <div class="relative w-full group" v-if="form.can_expire == 1">
                                    <label for="expire_at" class="block mb-2 text-sm font-medium text-gray-800">Expires At</label>
                                    <input type="date" id="expire_at" v-model="form.expire_at" class="box-border text-black placeholder-gray-400
                                text-sm rounded-lg bg-white border border-gray-200 border-solid
                                rounded outline-none focus:ring-indigo-400 focus:border-indigo-400 block w-full p-2.5"
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                form: {
                    id: '', form_method: '', expire_at: '',  purchase_id: '', item_id: '', can_expire: '', amount: 0, final_price: 0,
                      selling_price: 0, quantity: 0, updated_price: true, selling_price_set: true
                },
        ...
        methods: {
            async savePriceQuantity() {
                this.form.final_price = this.form.amount;
            ...
            setIsOpen(value, item = null) {
                ...
                if(item) {
                    ...
                    this.form.can_expire = item.can_expire;
                ...
                priceService.search({item_id: item.item_id}, (result)=>{
                    if(result.returnData != null){
                        this.listPrices = result.returnData.list_of_items;
                            ...
                            this.listPrices.selling_price = null;
                        }
